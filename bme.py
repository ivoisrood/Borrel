...
while True:
        ...
        bars = round(float(data['pressure']), 1)
        bar2 = str(round(float(data['pressure']), 1))
        if bar2 [0:1] == '9' :
                bar2=str(" "+bar2 [0:3])
        else:
                bar2=bars

        inch = round(bars * 29.53 * 0.001, 2)
        ...
        f=open('bme_log.txt', 'w')
        f.write(str(temp)+','+str(bar2)+','+str(humi))
        f.close()
        print(str(temp)+','+str(bar2)+','+str(humi))
        time.sleep(1)
